//-----------------------------------------------------------
// Ben Ebadinia
// Java++ compiler "global" definitions and the common classes
//    JAVAPLUSEXCEPTION, LISTER, READER, CODE, and IDENTIFIERTABLE
//
...
public:
   //-----------------------------------------------------------
   JAVAPLUSEXCEPTION(const char description[])
   //-----------------------------------------------------------
   {
...
//-----------------------------------------------------------
{
   ...
   strcat(this->sourceFileName,".java++");
   strcpy(fullFileName,sourceFileName);
   strcat(fullFileName,".list");
   LIST.open(fullFileName,ios::out);
   if ( !LIST.is_open() ) throw( JAVAPLUSEXCEPTION("Unable to open list file") );
   ListTopOfPageHeader();
}
...
*/
   ...
   pageNumber++;
   LIST << FF << '"' << sourceFileName << "\" 	Page " << setw(2) << pageNumber << endl;
...
//-----------------------------------------------------------
{
   ...
   if ( !SOURCE.is_open() ) throw( JAVAPLUSEXCEPTION("Unable to open source file") );
...
// index in [ 0,LOOKAHEAD ] where index = 0 means last GetNextCharacter() returned
   if ( (0 <= index) && (index <= LOOKAHEAD) )
      ...
      throw( JAVAPLUSEXCEPTION("GetLookAheadCharacter() index out-of-range") );
}
...
//-----------------------------------------------------------
{
   if ( numberCallbacks <= CALLBACKSALLOWED )
      ...
      throw( JAVAPLUSEXCEPTION("Too many callback functions") );
}
...
*/
   if ( identifiers > capacity )
      throw( JAVAPLUSEXCEPTION("Identifier table capacity exceeded") );
   else
   {
      identifiers++;
...
{
/*
   Java++ static data--global variable/constant definitions, string literals, and 
      ...
      Java++ program has been parsed.

   Java++ dynamic (frame-resident) data--subprogram module formal parameters and
...
//--------------------------------------------------
{
   ...
   if ( !STM.is_open() ) throw( JAVAPLUSEXCEPTION("Unable to open code file") );
}
...
//--------------------------------------------------
{
   ...
   EmitUnformattedLine("**--------------------------------------------------------------");
   sprintf(line,"** %s.stm",sourceFileName); 
   EmitUnformattedLine(line);
   EmitUnformattedLine("**--------------------------------------------------------------");
   ...
   EmitUnformattedLine("");
   EmitFormattedLine(""                   ,"JMP","STARTMAIN");
}
...
//--------------------------------------------------
{
   ...
   EmitUnformattedLine("");
   EmitUnformattedLine("**------------------------------------------------------------");
   EmitUnformattedLine("** Heap space for dynamic memory allocation ");
   EmitUnformattedLine("**------------------------------------------------------------");
